...
void MenuController_HandleInput(struct AppController *ctrl) {
    if (ctrl->app_state.current_screen == SCREEN_ABOUT || ctrl->app_state.current_screen == SCREEN_OBJECT) {
    ...
    if (ctrl->app_state.current_screen == SCREEN_MENU) {
        ...
        if (IsKeyPressed(KEY_DOWN) || IsKeyPressed(KEY_S)) {
            ctrl->menu_selected = (ctrl->menu_selected + 1) % 4;
        }
        if (IsKeyPressed(KEY_UP) || IsKeyPressed(KEY_W)) {
            ctrl->menu_selected = (ctrl->menu_selected - 1 + 4) % 4;
        }
        ...
        if (IsKeyPressed(KEY_ENTER) || IsKeyPressed(KEY_SPACE)) {
            switch(ctrl->menu_selected) {
                ...
                case 1: /* Tentang Aplikasi */
                    AppState_SetScreen(&ctrl->app_state, SCREEN_OBJECT);
                    break;
                case 2: /* Objek Aplikasi */
